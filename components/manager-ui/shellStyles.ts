/**
 * Sidebar style recipes for the manager shell — each side variant of
 * App_shell_navigation is pure data: container/background, text colors, and
 * which animated indicator it uses. Consumed by app/manager/layout.tsx (the
 * real shell) and by /theme-check-manager (static previews), so what the
...
export interface SideStyle {
  ...
  name: string;
  email: string;
  /** true → original-manager footer: user card + separate logout button. */

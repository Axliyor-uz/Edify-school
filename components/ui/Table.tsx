...
/* Usage:
   ...
   The look follows TEACHER_DESIGN.Table_style ('lined' | 'striped' | 'cards').
   <Table> provides the style through context so TRow/Td restyle themselves —
   existing call sites need zero changes.
*/

type TableStyle = TeacherDesignConfig["Table_style"];

const TableStyleContext = createContext<TableStyle>(TEACHER_DESIGN.Table_style);

export function Table({ className, children, ...rest }: HTMLAttributes<HTMLTableElement>) {
  const style = TEACHER_DESIGN.Table_style;
  return (
...
export function Pagination({ page, pageCount, onPage, className }: PaginationProps) {
  ...
  const btn = cn(
    ...
    TEACHER_DESIGN.Button_style === "pill" ? "rounded-full" : "rounded-m3-sm",
  );
  return (

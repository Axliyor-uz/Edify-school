// lib/SatSession.ts
//
// The SAT Math sitting being run, kept in the browser — same factory pattern
// as `lib/Examsession.ts::createSessionStore`, instantiated here for SAT's own
// snapshot shape (`SatExamSnapshot`) instead of touching that shared,
// Milliy/Rasch-depended-on module for the sake of one more type. Own
// localStorage key, so starting a SAT sitting can never discard a mock exam or
// a Milliy sertifikat paper still running against its clock, and vice versa.
//
...
export const subscribeSatSnapshot = (listener: () => void) => {
  listeners.add(listener);
  return () => listeners.delete(listener);
};

export function getSatSnapshot(): SatExamSnapshot | null {
...
export const getServerSatSnapshot = (): SatExamSnapshot | null => null;

export function saveSatSnapshot(snapshot: Omit<SatExamSnapshot, 'version' | 'savedAt'>): void {
...
export function clearSatSnapshot(): void {
  if (typeof window === 'undefined') return;
  try {

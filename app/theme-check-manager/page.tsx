...
/**
 * DEV-ONLY manager kit gallery — /theme-check-manager
 *
 * Unguarded, data-free preview of the manager design system so the switchboard
 ...
 * ManagerThemeProvider does:
 *
 *   /theme-check-manager?palette=wine&mode=dark&shape=round&density=airy
 *
 * Anything not overridden falls back to design.manager.config.ts. Safe to
...
import { managerThemeAttributes } from '@/components/manager-ui/config';
import { SIDE_STYLES, type SideStyle } from '@/components/manager-ui/shellStyles';
import { ALL_MANAGER_PALETTES } from '@/design.manager.config';
import {
  Plus, Search, Inbox, Users, TrendingUp, Zap, BookOpen,
  LayoutDashboard, Layers, Wallet, LogOut, Menu, CalendarCheck,
} from 'lucide-react';

const MODES = ['light', 'dark'] as const;

/** Static miniature of one sidebar recipe — same class bundles the real shell
 *  uses (components/manager-ui/shellStyles.ts), no framer/no auth needed. */
function MiniSidebar({ name, s }: { name: string; s: SideStyle }) {
  const items = [
    { icon: LayoutDashboard, label: 'Boshqaruv', active: false },
    { icon: Layers, label: 'Guruhlar', active: true },
    { icon: Wallet, label: "To'lovlar", active: false },
  ];
  return (
    <div className="rounded-m3-lg border border-outline-variant overflow-hidden bg-surface">
      <div className={cn('p-3 h-[228px] flex flex-col', s.bg)}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <b className={cn('text-[12px] font-bold truncate', s.brandTitle)}>Edify<span className={s.brandAccent}>Manager</span></b>
        </div>
...
const MOBILE_BAR_ITEMS = [
  { icon: LayoutDashboard, label: 'Asosiy', active: true },
  { icon: Layers, label: 'Guruhlar', active: false },
  { icon: CalendarCheck, label: 'Davomat', active: false },
  { icon: Wallet, label: "To'lovlar", active: false },
];
...
export default function ThemeCheckManager() {
  ...
  useEffect(() => {
    ...
    const attrs: Record<string, string> = { ...managerThemeAttributes(), 'data-mg-mode': 'light' };
    if (overrides.palette) attrs['data-mg-palette'] = overrides.palette;
    if (overrides.mode) attrs['data-mg-mode'] = overrides.mode;
    if (overrides.shape) attrs['data-mg-shape'] = overrides.shape;
    if (overrides.density) attrs['data-mg-density'] = overrides.density;
    if (overrides.font) attrs['data-mg-font'] = overrides.font;
    if (overrides.bg) attrs['data-mg-bg'] = overrides.bg;
    if (overrides.motion) attrs['data-mg-motion'] = overrides.motion;
    if (overrides.button) attrs['data-mg-button'] = overrides.button;
    for (const [k, v] of Object.entries(attrs)) el.setAttribute(k, v);
    el.style.colorScheme = attrs['data-mg-mode'];
    setStamped(true);
    return () => {
  ...
  const link = (patch: Record<string, string>) => {
    ...
    return `/theme-check-manager?${q.toString()}`;
  };
      ...
      <div className="max-w-5xl mx-auto px-t-page-x py-t-page-y space-y-t-gap-lg">
        ...
        <Card className="p-t-card space-y-2">
          <div className="flex flex-wrap gap-1.5">
            {ALL_MANAGER_PALETTES.map((p) => (
              <a key={p} href={link({ palette: p })}
                className={cn("px-3 py-1.5 rounded-m3-sm text-[12px] font-semibold border",
                  (params.palette || 'emerald') === p ? "bg-primary text-on-primary border-transparent" : "border-outline-variant text-on-surface-variant hover:bg-state-hover")}>
                {p}
              </a>
            ))}
          </div>
        ...
        <PageHeader title="Manager kit gallery" subtitle="Every component, current switchboard values" />
        ...
        <Card className="p-t-card space-y-3">
          ...
          <Banner tone="info" title="Heads up">Switchboard preview — change one word in design.manager.config.ts.</Banner>
        </Card>

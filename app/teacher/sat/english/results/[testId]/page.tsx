...
import { ArrowLeft, BookOpenText, ChevronDown, Target, TrendingUp, Users } from "lucide-react";

import { useTeacherLanguage } from "@/app/teacher/layout";
import { useAuth } from "@/lib/AuthContext";
import { SAT_RW_DOMAINS, type SatRwDomainInfo } from "@/lib/SatMathQuiz";
import { getSatEnglishTest, listSatEnglishResults } from "@/services/satEnglishQuizService";
...
import type { SatDomain, SatMathResult, SatMathTest, SatQuizItem } from "@/types/SatQuiz";
import type { Lang } from "@/types/Math";

/**
 * Who sat one SAT English test, and how they did. Sibling of
 * app/teacher/sat/math/results/[testId]/page.tsx.
 *
 ...
 * curve (`lib/SATscore.ts`), the way the real test actually works.
 *
...
export default function SatEnglishResultsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [testDoc, rows] = await Promise.all([
        getSatEnglishTest(params.testId),
        listSatEnglishResults(params.testId, user.uid),
      ]);
      setTest(testDoc);
      setResults(rows);
    } catch (err) {
  ...
  const domainRows = useMemo(() => {
    const totals: Partial<Record<SatDomain, { correct: number; total: number }>> = {};
    for (const r of results) {
      for (const [domain, v] of Object.entries(r.domains ?? {})) {
        const key = domain as SatDomain;
        const cur = totals[key] ?? { correct: 0, total: 0 };
        totals[key] = { correct: cur.correct + v.correct, total: cur.total + v.total };
      }
    }
    const rows: { domain: SatRwDomainInfo; stats: { correct: number; total: number } }[] = [];
    for (const domain of SAT_RW_DOMAINS) {
  ...
  if (!test) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        <EmptyState icon={<BookOpenText />} title={t.notFound} />
      </div>
    );
  }
    ...
    <div className="min-h-[100dvh] bg-surface pb-16">
      <div className="mx-auto flex max-w-4xl flex-col gap-6 px-4 py-6">
        <div className="flex items-center gap-2">
          <IconButton aria-label={t.back} size="sm" onClick={() => router.push("/teacher/sat/english")}>
            <ArrowLeft />
          </IconButton>

...
import {
  ArrowLeft, BarChart3, Check, Copy, KeyRound, Pencil, Plus, Sigma, Trash2,
} from "lucide-react";
...
import {
  deleteSatMathTest, listMySatMathTests, setSatTestStatus,
...
/**
 * The teacher's own SAT Math tests: code, status, edit, results, delete.
 * Contract: docs/SAT_QUIZ.md. Pattern: app/teacher/milliy-sertifikat/[subject]/page.tsx.
 */
...
const TR: Record<string, Record<string, string>> = {
  uz: {
    ...
    emptyDesc: "Har uchala modulni — Modul 1, Modul 2 Oson, Modul 2 Qiyin — o'zingiznikilardan yoki yangi yozib to'ldiring.",
  ...
  ru: {
    ...
    emptyDesc: "Заполните все три модуля — Модуль 1, Модуль 2 Лёгкий, Модуль 2 Трудный — своими или новыми вопросами.",
  ...
  en: {
    ...
    emptyDesc: "Fill all three modules — Module 1, Module 2 Easier, Module 2 Harder — from your own bank or by writing new questions.",
...
export default function SatMathTestsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setTests(await listMySatMathTests(user.uid));
    } catch (err) {
      console.error(err);
  ...
  const changeStatus = async (test: SatMathTest, status: SatMathTestStatus) => {
    ...
    try {
      await setSatTestStatus(test.id, status);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteSatMathTest(pendingDelete.id);

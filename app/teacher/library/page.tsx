...
const CardIllustration = ({ colorClass }: { colorClass: string }) => {
  return (
    <div className={`absolute inset-0 overflow-hidden rounded-m3-xl pointer-events-none opacity-40 group-hover:opacity-100 transition-opacity duration-700 ${colorClass}`}>
...
const TONES: Record<Tone, { tile: string; accent: string; chip: string }> = {
  primary: {
...
export default function LibraryHubPage() {
  ...
  const LIBRARY_CARDS: { id: string; icon: any; tone: Tone; href: string; data: any }[] = [
    { id: 'tests', icon: CheckSquare, tone: 'primary', href: '/teacher/library/tests', data: t.tests },
    { id: 'assessments', icon: FileText, tone: 'secondary', href: '/teacher/library/assessments', data: t.assess },
    { id: 'bank', icon: Database, tone: 'tertiary', href: '/teacher/create/my_questions', data: t.bank }, // 🟢 NEW QUESTION BANK ROUTE
  ];

  return (
    <div className="min-h-screen bg-surface flex flex-col font-t-body relative overflow-hidden pb-[100px]">
      ...
      <div className="absolute -left-[20%] top-[-10%] w-[70vw] h-[50vh] bg-[color-mix(in_oklab,var(--m3-primary)_5%,transparent)] rounded-full blur-[120px] pointer-events-none z-0"></div>
      <div className="absolute -right-[20%] top-[10%] w-[70vw] h-[50vh] bg-[color-mix(in_oklab,var(--m3-tertiary)_5%,transparent)] rounded-full blur-[120px] pointer-events-none z-0"></div>
      ...
      <div className="flex-1 flex flex-col items-center w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 relative z-10 pt-8 md:pt-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10 md:mb-14 max-w-2xl flex flex-col items-center">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface-container-low shadow-elev-1 text-on-surface-variant font-bold text-[10px] md:text-[11px] uppercase tracking-widest mb-4">
            <BookCopy size={14} className="text-primary" /> {t.heroBadge}
          </div>
          <h1 className="text-2xl md:text-4xl font-black text-on-surface tracking-tight mb-3 leading-tight">{t.title}</h1>
          <p className="text-on-surface-variant text-[13px] md:text-[15px] font-medium leading-relaxed px-4">{t.subtitle}</p>
        </motion.div>
          ...
          {LIBRARY_CARDS.map((item) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-surface-container-low rounded-m3-lg md:rounded-m3-xl p-5 md:p-7 shadow-elev-1 hover:shadow-elev-2 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col"
              >
                <CardIllustration colorClass={tone.accent} />

                {/* Header: Icon & Badge */}
                <div className="flex justify-between items-start mb-5 md:mb-6 relative z-10">
                  <div className={`w-12 h-12 md:w-14 md:h-14 rounded-m3-md flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 shrink-0 ${tone.tile}`}>
                    <item.icon size={24} strokeWidth={2.5} className="md:w-7 md:h-7" />
                  </div>
                  <span className={`text-[9px] md:text-[10px] font-black px-2.5 py-1.5 rounded-m3-xs uppercase tracking-widest ${tone.chip}`}>{item.data.badge}</span>
                </div>

                {/* Text Content */}
                <div className="relative z-10 flex-1">
                  <h2 className="text-[18px] md:text-[20px] font-black text-on-surface group-hover:text-primary transition-colors leading-tight mb-2 md:mb-3">{item.data.title}</h2>
                  <p className="text-[12px] md:text-[14px] text-on-surface-variant font-medium leading-relaxed mb-5 md:mb-6 line-clamp-3">{item.data.desc}</p>

                  {/* Features Pills */}
                  <div className="flex flex-wrap gap-1.5 md:gap-2 mb-5 md:mb-6">
                    {item.data.features.map((feature: string, i: number) => (
                      <span key={i} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-m3-xs text-[10px] md:text-[11px] font-bold ${tone.chip}`}>
                ...
                {/* Bottom Action */}
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-outline-variant relative z-10 transition-colors group-hover:border-transparent">
                  <span className={`text-[12px] md:text-[13px] font-black transition-colors ${tone.accent}`}>{item.data.btn}</span>
                  <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center transition-colors shadow-elev-1 bg-surface-container-lowest border border-outline-variant group-hover:bg-transparent group-hover:border-transparent ${tone.accent}`}>

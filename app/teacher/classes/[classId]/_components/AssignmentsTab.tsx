...
import {
  ...
  Copy, AlertTriangle, X, Play, Lock, CheckCircle, User as UserIcon
} from 'lucide-react';
import toast from 'react-hot-toast';
import { useTeacherLanguage } from '@/app/teacher/layout';
import { useRouter } from 'next/navigation';
import { Button, IconButton, Spinner, StatusChip, EmptyState } from '@/components/ui';
    ...
    <>
      <div className="space-y-3 md:space-y-4">
        {assignments.length === 0 && (
          <div className="py-8 bg-surface-container rounded-m3-lg border border-dashed border-outline-variant">
            <EmptyState
              icon={<Clock />}
              title={t.emptyTitle}
              action={
        ...
        <div className="grid gap-3">
          {assignments.map((a, index) => {
            ...
            const statusUI = {
              scheduled: { bg: 'bg-primary-container', text: 'text-on-primary-container', tone: 'info' as const, icon: <Clock size={16}/> },
              active: { bg: 'bg-success-container', text: 'text-on-success-container', tone: 'success' as const, icon: <Play size={16}/> },
              closed: { bg: 'bg-surface-container-highest', text: 'text-on-surface-variant', tone: 'muted' as const, icon: <Lock size={16}/> }
            };
            ...
            return (
              <div
                ...
                className="bg-surface-container-low rounded-m3-lg border border-outline-variant p-4 hover:border-outline hover:shadow-elev-1 transition-all duration-200 cursor-pointer group flex flex-col md:flex-row md:items-center gap-4 active:scale-[0.98] md:active:scale-100"
              >
                <div className="flex items-start gap-3 md:gap-4 flex-1 min-w-0">
                  <div className={`w-10 h-10 rounded-m3-md flex items-center justify-center shrink-0 ${statusUI[status].bg} ${statusUI[status].text}`}>
                    {statusUI[status].icon}
                  </div>

                  <div className="flex-1 min-w-0 pt-0.5">
                    <div className="flex items-center gap-2">
                       <h3 className="font-bold text-[14px] md:text-[15px] text-on-surface group-hover:text-primary transition-colors truncate">
                         {a.testTitle || 'Untitled Assignment'}
                       </h3>
                       <StatusChip tone={statusUI[status].tone} className="hidden sm:inline-flex">
                         {t.status[status]}
                       </StatusChip>
                    </div>

                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-[11px] md:text-[12px] font-bold text-on-surface-variant">
                      {dueDate ? (
                        <span className={`flex items-center gap-1 ${status === 'closed' ? 'text-error' : ''}`}>
                          <Calendar size={12}/> {t.status.due} {dueDate.toLocaleDateString([], {month: 'short', day: 'numeric'})}
                        </span>
                      ) : <span className="text-on-surface-variant">{t.status.noDeadline}</span>}
                ...
                <div className="flex items-center justify-between md:justify-end gap-4 md:gap-6 w-full md:w-auto border-t md:border-t-0 pt-3 md:pt-0 border-outline-variant pl-14 md:pl-0">
                   <div className="flex flex-col w-32 shrink-0">
                     <div className="flex justify-between items-end mb-1.5">
                        <span className="text-[10px] md:text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">{submittedCount}/{targetRequired} {t.progress.submitted}</span>
                        <span className={`text-[10px] md:text-[11px] font-black ${percent === 100 ? 'text-success' : 'text-on-surface'}`}>{Math.round(percent)}%</span>
                     </div>
                     <div className="w-full h-1.5 bg-surface-container-highest rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-500 ${percent === 100 ? 'bg-success' : 'bg-primary'}`} style={{ width: `${percent}%` }}></div>
                     </div>
                   </div>

                   <div className="flex items-center gap-1 shrink-0">
                      <IconButton aria-label="Edit" size="sm" title="Edit" onClick={(e) => { e.stopPropagation(); onEdit(a); }} className="hover:text-primary"><Edit2 strokeWidth={2.5}/></IconButton>
                      <IconButton aria-label="Copy Link" size="sm" title="Copy Link" onClick={(e) => handleCopyLink(e, a.id)}><Copy strokeWidth={2.5}/></IconButton>
                      <IconButton aria-label="Delete" size="sm" title="Delete" onClick={(e) => { e.stopPropagation(); setDeleteData(a); }} className="hover:text-error"><Trash2 strokeWidth={2.5}/></IconButton>
                   </div>
                </div>
              </div>
            )
      ...
      {mounted && deleteData && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-scrim backdrop-blur-sm transition-opacity" onClick={() => setDeleteData(null)}></div>
          <div className="relative bg-surface-container-low rounded-m3-xl w-full max-w-sm p-6 md:p-8 shadow-elev-3 animate-in zoom-in-95 fade-in text-center">
             <div className="w-14 h-14 bg-error-container text-on-error-container rounded-m3-lg flex items-center justify-center mb-5 mx-auto"><AlertTriangle size={24} strokeWidth={2.5}/></div>
             <h3 className="text-[18px] font-black text-on-surface mb-2">{t.modals.deleteTitle}</h3>
             <p className="text-[13px] text-on-surface-variant font-medium mb-6 leading-relaxed">{t.modals.deleteDesc.replace("{title}", deleteData.testTitle)}</p>
             <div className="flex flex-col-reverse sm:flex-row gap-3">
               <Button variant="tonal" size="lg" onClick={() => setDeleteData(null)} className="w-full">{t.modals.cancel}</Button>
               <Button variant="danger" size="lg" onClick={handleDeleteConfirm} className="w-full">{t.modals.confirmDelete}</Button>
             </div>
          </div>
        </div>,
        document.body
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center p-0 sm:p-6">
      ...
      <div className="relative bg-surface-container-low rounded-t-m3-xl sm:rounded-m3-xl w-full max-w-lg h-[90vh] sm:h-[80vh] flex flex-col shadow-elev-3 animate-in slide-in-from-bottom-10 sm:zoom-in-95 fade-in duration-300 overflow-hidden">
        ...
        {/* Header */}
        <div className="p-5 md:p-8 border-b border-outline-variant flex items-start justify-between shrink-0 bg-[color-mix(in_oklab,var(--m3-surface-container-low)_90%,transparent)] backdrop-blur-md z-20 shadow-elev-1">
          <div className="flex-1 pr-4 min-w-0">
            <h2 className="text-[16px] md:text-[18px] font-black text-on-surface leading-tight mb-3 truncate">{assignment.testTitle}</h2>
            <div className="bg-surface-container border border-outline-variant rounded-m3-md p-3 md:p-4">
              <div className="flex justify-between items-end mb-2">
                <span className="text-on-surface-variant text-[10px] md:text-[11px] font-bold uppercase tracking-widest">{submittedCount}/{targetRequired} {t.progress.submitted}</span>
                <span className="text-[12px] md:text-[13px] font-black text-primary">{Math.round(percent)}%</span>
              </div>
              <div className="w-full h-1.5 bg-surface-container-highest rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-1000 ${percent === 100 ? 'bg-success' : 'bg-primary'}`} style={{ width: `${percent}%` }}></div>
              </div>
            </div>
          </div>
          <IconButton aria-label="Yopish" size="sm" onClick={onClose} className="shrink-0"><X strokeWidth={2.5}/></IconButton>
        </div>

        {/* Segmented Tabs */}
        <div className="px-5 md:px-8 py-3 md:py-4 bg-surface shrink-0 z-10 border-b border-outline-variant">
          <div className="flex p-1 bg-surface-container-high rounded-m3-md shadow-inner">
            <button onClick={() => setActiveTab('submitted')} className={`flex-1 py-2 text-[11px] md:text-[12px] font-bold rounded-m3-sm transition-all ${activeTab === 'submitted' ? 'bg-surface-container-lowest text-primary shadow-elev-1 ring-1 ring-outline-variant' : 'text-on-surface-variant hover:text-on-surface'}`}>
              {t.submissions.submitted} ({submittedCount})
            </button>
            <button onClick={() => setActiveTab('pending')} className={`flex-1 py-2 text-[11px] md:text-[12px] font-bold rounded-m3-sm transition-all ${activeTab === 'pending' ? 'bg-surface-container-lowest text-primary shadow-elev-1 ring-1 ring-outline-variant' : 'text-on-surface-variant hover:text-on-surface'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-surface custom-scrollbar">
          {loading ? (
            <div className="py-20 flex justify-center"><Spinner size={28} /></div>
          ) : displayList.length === 0 ? (
            <div className="py-16 flex flex-col items-center text-on-surface-variant gap-3 border-2 border-dashed border-outline-variant rounded-m3-lg bg-surface-container-lowest shadow-elev-1">
              ...
              {displayList.map((student, idx) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center gap-2.5 md:gap-3 p-3.5 md:p-4 bg-surface-container-lowest border border-outline-variant rounded-m3-lg shadow-elev-1 hover:border-primary hover:shadow-elev-2 active:scale-[0.98] transition-all cursor-pointer group"
                  >
                    {activeTab === 'submitted' && (
                    ...
                    <div className="w-9 h-9 md:w-10 md:h-10 bg-surface-container-high rounded-m3-md flex items-center justify-center font-black text-on-surface-variant text-[13px] md:text-[14px] shrink-0 border border-outline-variant overflow-hidden">
                      {avatarUrl ? (
                    ...
                    <div className="flex-1 min-w-0 pr-2">
                      <p className="font-black text-on-surface text-[13px] md:text-[14px] truncate group-hover:text-primary transition-colors">{student.displayName}</p>
                      <p className="text-[10px] md:text-[11px] font-bold text-on-surface-variant truncate">@{student.username || 'student'}</p>
                    </div>
                    {activeTab === 'submitted' && attempt ? (
                      <div className="text-right shrink-0">
                        <p className={`font-black text-[15px] md:text-[16px] leading-none ${scoreP >= 60 ? 'text-success' : 'text-error'}`}>{scoreP}%</p>
                        <p className="text-[9px] md:text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mt-1">{attempt.score}/{attempt.totalQuestions} {t.submissions.correct}</p>
                      </div>
                    ) : activeTab === 'pending' ? (
                      <StatusChip tone="muted" className="shrink-0">{t.submissions.pending}</StatusChip>
                    ) : null}
                  </div>
                )
              })}

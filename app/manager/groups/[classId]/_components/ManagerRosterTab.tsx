...
import ManagerAddStudentModal from "./ManagerAddStudentModal";
...
const TRANSLATIONS = {
  uz: {
    ...
    removed: (name: string) => `${name} guruhdan o'chirildi.`,
    removeError: "O'quvchini o'chirishda xatolik yuz berdi.",
    emptyTitle: "Guruh bo'sh",
    ...
    removeFromGroupTitle: "Guruhdan o'chirish",
    confirmTitle: "O'quvchini o'chirish",
    confirmMessage: (name: string) => `${name} guruhdan o'chiriladi. Uni keyinchalik qayta qo'shishingiz mumkin.`,
    confirmLabel: "O'chirish",
  },
  en: {
    unknownName: "Unknown",
    unknownUsername: "unknown",
    removed: (name: string) => `${name} was removed from the group.`,
    removeError: "Something went wrong while removing the student.",
    emptyTitle: "The group is empty",
    ...
    confirmMessage: (name: string) => `${name} will be removed from the group. You can add them back later.`,
    confirmLabel: "Remove",
  },
  ru: {
    unknownName: "Неизвестно",
    unknownUsername: "неизвестно",
    removed: (name: string) => `${name} удалён(а) из группы.`,
    removeError: "Не удалось удалить ученика.",
    emptyTitle: "Группа пуста",
    ...
    removeFromGroupTitle: "Удалить из группы",
    confirmTitle: "Удаление ученика",
    confirmMessage: (name: string) => `${name} будет удалён(а) из группы. Позже его можно добавить снова.`,
...
export default function ManagerRosterTab({ classId, centerId, studentIds, ieltsGroupId, onRefresh }: RosterProps) {
  ...
  useEffect(() => {
    ...
    async function fetchStudents() {
      ...
      try {
        ...
        const results = (await Promise.all(studentPromises)).filter(Boolean) as StudentProfile[];
  ...
  const handleRemoveStudent = async () => {
    ...
    try {
      if (ieltsGroupId) {
        await managedRosterBatch(classId, ieltsGroupId, studentToRemove.uid, "remove");
      } else {
        const classRef = doc(db, "classes", classId);
        await updateDoc(classRef, { studentIds: arrayRemove(studentToRemove.uid) });
      }
      ...
      {filtered.length === 0 ? (
        ...
        <div className="divide-y divide-outline-variant">
          {filtered.map((student) => (
            <div
              key={student.uid}
              className="flex items-center gap-3 px-4 sm:px-5 py-3 hover:bg-state-hover transition-colors"
            >
              ...
              <div className="min-w-0 flex-1">
                <h4 className="text-[14.5px] font-semibold text-on-surface truncate">
                  {student.displayName}
                </h4>

...
export default function TeacherSignupFlow({ onBack, t, googleMode = false }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 2) {
      if (!formData.fullName || !formData.username || !formData.schoolSubject) return toast.error(t.validation.fillAll);
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      batch.set(doc(db, "users", user.uid), {
        ...
        role: "teacher", 
        subject: formData.schoolSubject,
        
        // Progressive Profiling Empty Fields
        phone: "", 
        birthDate: "", 
        gender: "", 
        institution: "",
      ...
      toast.success(t.validation.welcome.replace("{role}", "Ustoz"));
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full">
      ...
      {step === 2 && <Step2Profile formData={formData} handleChange={handleChange} usernameError={usernameError} usernameAvailable={usernameAvailable} isCheckingUser={isCheckingUser} role="teacher" t={t} />}

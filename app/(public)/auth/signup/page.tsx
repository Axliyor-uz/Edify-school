...
import ManagerSignupFlow from "./_components/ManagerSignupFlow";
...
export const SIGNUP_TRANSLATIONS = {
  uz: {
    ...
    steps: {
      ...
      google: { title: "Profilni to'ldiring", sub: "Google hisobingiz ulandi — parol kerak emas" }
    },
    roles: {
      ...
      manager: { title: "Men Menejerman", desc: "O'quv markazini ro'yxatdan o'tkazish va boshqarish." }
    },
    inputs: { email: "Email manzili", username: "Foydalanuvchi nomi", password: "Parol", fullname: "To'liq ism (F.I.O)", phone: "Telefon raqam", centerName: "Markaz nomi", studentCount: "O'quvchilar soni" },
  ...
  en: {
    ...
    heroSub: "TestEdify — a modern education platform. A seamless digital environment for teachers and students.",
    steps: {
      ...
      google: { title: "Complete your profile", sub: "Your Google account is connected — no password needed" }
    },
    roles: {
      ...
      manager: { title: "I am a Manager", desc: "Register a learning center and manage operations." }
    },
    inputs: { email: "Email Address", username: "Username", password: "Password", fullname: "Full Name", phone: "Phone Number", centerName: "Center Name", studentCount: "Student Count" },
  ...
  ru: {
    ...
    heroSub: "TestEdify — современная образовательная платформа. Единая цифровая среда для учителей и учеников.",
    steps: {
      ...
      google: { title: "Заполните профиль", sub: "Аккаунт Google подключён — пароль не нужен" }
    },
    roles: {
      ...
      manager: { title: "Я Менеджер", desc: "Зарегистрировать учебный центр и управлять им." }
    },
    inputs: { email: "Эл. почта", username: "Имя пользователя", password: "Пароль", fullname: "Ф.И.О", phone: "Телефон", centerName: "Название центра", studentCount: "Кол-во учеников" },
  ...
  return (
    <div className="min-h-screen flex font-['Inter',sans-serif]">
      ...
      <div className="hidden lg:flex w-[55%] bg-gradient-to-br from-[#0A2540] via-[#1E3A8A] to-[#1565C0] relative overflow-hidden items-center justify-center p-10">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: "easeOut" }} className="relative z-10 w-full max-w-[520px]">
          <EcosystemSVG />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.35 }} className="absolute bottom-10 left-10 right-10 z-10">
          <h2 className="text-white text-[26px] font-black leading-tight tracking-tight mb-2">{t.heroTitle}</h2>
          <p className="text-blue-100/55 text-[14px] font-medium leading-relaxed max-w-md">{t.heroSub}</p>
        </motion.div>
      ...
      {/* RIGHT PANEL — 45% */}
      <div className="flex-1 flex flex-col justify-center bg-white px-6 sm:px-12 py-12 relative overflow-y-auto">
        {/* Mobile logo */}
          ...
          <AnimatePresence mode="wait">
            {!role && (
              <motion.div key="step0" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}>
                ...
                {/* New Google users finish onboarding (role, username, …) at
                    /auth/complete-profile; existing ones go straight to their dashboard */}
            ...
            {role === "manager" && (
              <motion.div key="manager" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
                <ManagerSignupFlow onBack={() => setRole(null)} t={t} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
